...
def quick_bench():
    b = Bencher(LRUCache, MAXTIME=2).bench(BSIZES=(2**7, 2**12, 2**14, 2**15, 2**16,))
    b = Bencher(LinkedListLRUCache, MAXTIME=2).bench(BSIZES=(2**7, 2**12, 2**14, 2**15,))
    b = Bencher(SmallLRUCache, MAXTIME=2).bench(BSIZES=(2**7, 2**12, 2**14, 2**15,))

def slow_bench():
    b = Bencher(LRUCache, MAXTIME=5).bench(BSIZES=[2**x for x in range(7, 21)])
    b = Bencher(LinkedListLRUCache, MAXTIME=5).bench(BSIZES=[2**x for x in range(7, 21)])
    b = Bencher(SmallLRUCache, MAXTIME=5).bench(BSIZES=[2**x for x in range(7, 17)])
...
class Testy(unittest.TestCase):
    ...
    def _mem_test_much_adding_some_removing(self):
        for klass in (LRUCache, SmallLRUCache,):
